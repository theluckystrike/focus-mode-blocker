...
async function handleMessage(message, sender) {
  switch (message.type) {
    ...
    case 'START_BREAK':
      await startBreak(message.isLong || false);
      return { success: true };
...
async function getFullState() {
  const timerState = await getTimerState();
  const stats = await getTodayStats();
  const { streak, settings, blocklist, activePrebuiltLists, sessionCount } = await getStorage([
    'streak', 'settings', 'blocklist', 'activePrebuiltLists', 'sessionCount'
  ]);
...
async function handleUpdateBlocklist(sites) {
  ...
  await setStorage({ blocklist: sites });
...
chrome.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === 'install') {
    ...
    await chrome.alarms.create(ALARM_SCHEDULE_CHECK, { periodInMinutes: 1 });
  }
